...
`origin` は https://github.com/HAMAKIN05/AtlasQuarry.git 。
**コミットで止めず、その作業のうちに `git push origin main` まで済ませる。**
手元にしか無い状態を残さない（本番は動いているのにリポジトリが古い、が起きる）。

ただし **`git add -A` は使わない。** `.env` の退避や DB ダンプが置いてある状態で
やると秘匿情報ごと上がる（前科あり）。**変更したパスを明示して `add` する。**
